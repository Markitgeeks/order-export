...
/* 🔹 LOADER → GET request ke liye (frontend me use hota hai) */
export async function loader({ request }) {
  ...
  // DB se existing orders fetch karo
  await connectDB();
...
/* 🔹 ACTION → POST request ke liye (Shopify webhook ya internal call) */
export async function action({ request }) {
  try {
    const body = await request.json();
    const { shop, orders } = body;

    if (!orders || !Array.isArray(orders)) {
      return json({ error: "Orders data missing" }, { status: 400 });
    }
...
/* 🔹 FRONTEND COMPONENT → Orders ko UI me show karna */
export default function AppOrder() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const res = await fetch("/app/order");
